...
For silent installs, or if you clear the checkbox, a **Start Menu → Pi Dash → "Sign in to Pi Dash"** shortcut runs the same flow. From a terminal, bare `pidash` with no config also drops into `auth login`.
