        ...
        tokio::select! {
            ...
            req = kill_rx.recv() => {
                match req {
                    ...
                    None => {
                        // All senders dropped — recv resolves to None
                        // synchronously every iteration. Stop polling the recv
                        // arm and just await natural exit, otherwise the biased
                        // select! would spin on the closed channel without ever
                        // polling child.wait().
