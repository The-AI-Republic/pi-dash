...
impl AgentBridge {
    ...
    pub async fn spawn_from_config_with_resume(
        ...
        match runner.agent.kind {
            ...
            AgentKind::CursorAgent => {
                let b = crate::cursor_agent::bridge::Bridge::spawn_with_resume(
                    &runner.cursor_agent.binary,
                    cwd,
                    selected_model(model_override, runner.cursor_agent.model_default.clone()),
                    resume_session_id,
                )
                .await?;
                Ok(AgentBridge::CursorAgent(b))
            }
            AgentKind::OpenClaw => {
                let b = crate::openclaw::bridge::Bridge::spawn_with_resume(
                    &runner.openclaw.binary,
                    cwd,
                    selected_model(model_override, runner.openclaw.model_default.clone()),
                    resume_session_id,
                )
                .await?;
    ...
    pub async fn run_one_shot(&mut self, payload: &RunPayload, cwd: &Path) -> Result<AgentCursor> {
        match self {
            ...
            AgentBridge::CursorAgent(b) => Ok(AgentCursor::CursorAgent(
                b.run_one_shot(payload, cwd).await?,
            )),
