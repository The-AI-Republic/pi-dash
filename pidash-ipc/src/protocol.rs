...
/// string we don't understand. Pub because the daemon's auto-swap gate
/// uses the same compare to decide whether to swap.
...
mod tests {
    ...
    fn observability_field_roundtrips() {
        use crate::dto::{ObservabilitySnapshot, TokenUsage};
        let snap = StatusSnapshot {
            daemon: DaemonInfo {
                cloud_url: "https://x".into(),
                connected: true,
