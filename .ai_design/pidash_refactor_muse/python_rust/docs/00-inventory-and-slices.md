...
| `api/v1/`        | `api.urls` (16 modules)                         | external API tokens; CLI device-code login + machine tokens (`api/v1/auth/`, `api/urls/auth.py`) |
| `api/v1/runner/` | `runner.urls`                                   | runner agents (machine tokens), desktop                                                          |
| `auth/`          | `authentication.urls`                           | web, admin, space (`spaces/…`)                                                                   |
...
| `loop`            | Auto-PM jobs (`users/me/auto-pm/…`, `loop/urls.py`, `loop/views.py`)                                                                                                                                                                                                                                                                                                                                   | 9 files, 764 LOC                                                                                                                             | 2 / 2 / ~1 / `LoopJob`, `LoopTarget`, `LoopUserPreference` in `db`                                                                                                                                              | 0 (served by `bgtasks/loop.py`) / 0; `tests/contract/loop`                                                                  | web                                                                    | Medium: `app`, `assistant`, `bgtasks`, `db`, `license`                                                               | Low                                                                                                                                                              |
...
`bgtasks/github_signals.py` bridges git events into tasks (`bgtasks/scheduler.py` is a task module with no receivers);
...
- **django-filter** (`FilterSet` 28, `ComplexFilterBackend`, `IssueFilterSet` in `utils/filters/{filterset,filter_backend}.py`).
...
2. `ee/` stubs (6 modules at `ddaef0c2`, incl. `ee/assistant/stt_provider.py`): `ee/assistant/model_provider.py`,
...
| F-10 | Extension seams for the overlay             | Traits with default impls for every `ee/` stub (6 at `ddaef0c2`; re-count at filing) (§4 item 2), named route-group replacement in the app builder (§4 item 3), private migration directory convention                                                                   | F-03, F-06 |
...
| D-01 | 1     | license / instance console                            | `api/instances/` · 15                           | `license/`                                                                                                                                                                                                                                                                                   | `Instance`, `InstanceAdmin`, `InstanceConfiguration`, `ChangeLog` | none                                                                          | `full`                                                                                     |
| D-02 | 2     | space                                                 | `api/public/` · 25                              | `space/`                                                                                                                                                                                                                                                                                     | reads `db` (deploy boards, anchors, assets)                       | none                                                                          | `full`                                                                                     |
| D-03 | 3     | loop                                                  | `api/users/me/auto-pm/` · 2                     | `loop/`, `bgtasks/loop.py`                                                                                                                                                                                                                                                                   | `LoopJob`, `LoopTarget`, `LoopUserPreference`                     | `tests/contract/loop`                                                         | `full`                                                                                     |
...
| D-10 | 7     | bgtasks: agent ticker + scheduler workers             | — (tasks + beat)                                | `bgtasks/{agent_ticker,scheduler,_rrule}.py` (`bgtasks/loop.py` is owned by D-03), `celery.py` beat schedule                                                                                                                                                                                 | `IssueAgentTicker`, `Scheduler*`                                  | `tests/unit/bg_tasks`, `tests/unit/test_celery_schedule.py`                   | `full`                                                                                     |
...
| D-12 | 9     | orchestration engine                                  | — (signals)                                     | `orchestration/` (incl. `blockers.py`, `done_signal.py`, `relations.py`, `workpad.py`, `service.py`, `scheduling.py`), `core/` remainder                                                                                                                         | reads `db`, `runner`                                              | `tests/unit/orchestration`                                                    | `full`                                                                                     |
| D-13 | 10    | runner: enrollment + auth + machines                  | `api/v1/runner/` (enrollment, tokens, machines) | `runner/authentication.py`, `runner/services/{tokens,validation,runner_delete,pod_naming}.py`, `runner/views/{enrollment,register,runners,pods,projects,desktop,machine_commands}.py`                                                                                                        | `Runner`, `DevMachine`, `MachineToken`, `RunnerForceRefresh`      | `tests/contract/runner`, `tests/unit/runner`                                  | `full`                                                                                     |
...
| D-17 | 11    | authentication: OAuth                                 | `auth/` (google/github/gitlab/gitea)            | `authentication/provider/oauth/`, `authentication/views/{app,space}/` OAuth views                                                                                                                                                                                                            | `SocialLoginConnection`                                           | same                                                                          | `full`                                                                                     |
...
| D-22 | CLI auth + runner v1                 | `auth` 6 (`api/v1/auth/`: device start/approve/token, revoke, machine token, workspaces), `runner` 1 | `authentication/views/cli/`, `authentication/services/cli_tokens.py`, `api/views/runner.py`                                                                                                                               | `CLIDeviceCode` model; no dedicated tests today                                                                                                | `full` — wire-compatible with installed `pidash` CLIs; belongs with slice 12 because the routes are under `api/v1/` |
...
- Small: ~1.9k LOC, 15 routes, 14 views, 7 serializers, 4 models (`Instance`, `InstanceAdmin`, `InstanceConfiguration`, `ChangeLog`) — reviewable in one PR.
...
- Few model dependencies: the four `license` models (+ shared `User` reads), of which `InstanceConfiguration` is also read by F-03; small enough that the
...
- Success bar: byte-equivalent responses on the contract suite for all 15 routes, dual-run with
  Django owning the four `license` tables, rollback by routing change only.
