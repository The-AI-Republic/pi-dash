...
- **Evict any prior active session** for this connection: mark
  ...
  it to return `409 session_evicted`. The old session's per-runner
  Redis streams (`runner_stream:{old_sid}:{rid}`) are **not** moved
  here — those streams are still keyed by the old `sid` and are kept
  alive for `2 × access_token_ttl_secs` post-eviction (§7.4
  retention contract). Decision #16.
- Create a new `RunnerSession` row.
- Return the synchronous `Welcome` payload.

**XCLAIM happens at `attach/` time, not here.** The new session's
per-runner stream key (`runner_stream:{new_sid}:{rid}`) only exists
once the new daemon calls `attach/` for that runner; only then is
there a target to claim into. See §7.2 for the per-runner handoff
sequence.
...
**Per-runner Redis stream materialization and PEL claim happen here**,
in this exact order, in a single Redis transaction:

a. Create the new session's per-runner stream/group:
`XGROUP CREATE runner_stream:{sid}:{rid} daemon-{sid} $ MKSTREAM`
(idempotent; ignore `BUSYGROUP` errors for retries).
b. Look up any prior session for this connection that was revoked in
the last `2 × access_token_ttl_secs` and check whether it had a
per-runner stream `runner_stream:{old_sid}:{rid}` with a non-empty
PEL. If so, `XCLAIM IDLE 0 ... <pel_ids>` from the old stream into
the new one. (This is the per-runner half of §7.1's session-eviction
handoff.)
c. Drain the offline-stream buffer: `XRANGE runner_offline_stream:{rid} - +`,
for each entry `XADD` to `runner_stream:{sid}:{rid}` (preserving
the original offline id in a metadata field), then `XDEL` from
the offline stream. Bounded by §7.4's offline cap (1000 entries
per runner). Atomic per entry; partial failure leaves the offline
stream in a recoverable state.
d. Return `welcome`, the new `stream_id`, and `starting_id` (the id
from which the new daemon should begin reading; typically the
first claimed/copied entry id, or `0-0` if there was no prior
state).

If the daemon's `attach/` call fails after step (a) but before the
daemon begins polling, the empty stream + group are reaped by the
session-cleanup task on session-eviction or session-DELETE; no
dangling state.
...
offline stream). Any entries already in the per-runner stream's PEL
are retained for the standard 2×TTL window in case the runner
re-attaches.
...
This shape supports messages-retained-until-ack and per-runner
cursors, which a list-based outbox with a single scalar cursor
cannot.
...
def enqueue_for_runner(runner_id, msg):
    ...
    if sid is None:
        ...
        #   work queues for offline runners; scheduler must re-match
        #   against an attached runner instead of targeting this one.
...
Offline-stream handoff on attach:

1. `attach/` drains `runner_offline_stream:{rid}` oldest-first.
2. Each entry is copied into `runner_stream:{sid}:{rid}` with a field
   preserving the original offline stream id for audit/debug.
3. After successful copy, the offline entry is deleted.
4. If the offline stream exceeded TTL/cap before attach, dropped
   messages are surfaced in logs/metrics; they are **not** replayed
   from another store.
...
is what makes the at-least-once delivery property correct.

Retention / cleanup contract:

- Active session streams use `MAXLEN ~ 10000` entries per
  `(session_id, runner_id)` stream. This is a safety cap, not the
  primary ack mechanism.
- On `session DELETE` or `session_evicted`, the old session's streams
  are retained for `2 × access_token_ttl_secs` to allow claim/replay,
  then deleted by a cleanup task if the session remains revoked.
- Streams with no attached runner and empty PEL for >24h are deleted.
...
- **PEL handoff is not done here.** Pending entries from the old
  session live on the old session's per-runner streams
  (`runner_stream:{old_sid}:{rid}`) until the next attach for that
  runner under the new session, at which point §7.2 step (b) does
  the per-runner `XCLAIM`. The old streams are retained for
...
- `sweep_old_streams` (every 5 min): scan revoked sessions older
  than `2 × access_token_ttl_secs` and `XGROUP DESTROY` + `DEL`
  their per-runner streams. Also delete offline streams with
