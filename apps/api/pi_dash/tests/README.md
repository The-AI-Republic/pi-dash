...
This directory contains tests for the Pi Dash application. The tests are organized using pytest.
...
Pi Dash has two types of API endpoints:

1. **External API** (`pi_dash.api`):
...
```bash
# Run unit tests
python -m pytest pi_dash/tests/unit/

# Run API contract tests
python -m pytest pi_dash/tests/contract/api/

# Run App contract tests
python -m pytest pi_dash/tests/contract/app/

# Run smoke tests
python -m pytest pi_dash/tests/smoke/
```
...
- `pi_dash_server`: Live Django test server for HTTP-based smoke tests
...
2. Use the correct client fixture based on the API being tested:
   ...
   - For smoke tests with real HTTP, use `pi_dash_server`
...
```bash
python -m pytest --cov=pi_dash --cov-report=term --cov-report=html
```
