...
@pytest.fixture(autouse=True)
def _stub_drain():
    """Pause/resume-unavailable handlers schedule drain via ``on_commit``;
    the ``_on_commit_immediate`` fixture runs those synchronously. Without
    stubbing, drain reassigns the run we just queued and the
    ``status==QUEUED`` assertion flips to ``ASSIGNED``. Drain itself is
    covered by test_drain_pod.py.
    """
    with patch(
        "pi_dash.runner.services.matcher.drain_pod_by_id"
    ), patch(
...
def _consumer_for(runner):
    # Sync helpers (_handle_run_paused, _handle_resume_unavailable, …) take
    # the Runner as an arg, so we don't need to populate the consumer's
    # internal connection/authorised_runners state for these tests.
...
@pytest.mark.xfail(
    reason=(
        "Pending: pause-sweep continuation feature is not implemented in "
        "orchestration/service.py — only this test references it. Re-enable "
        "when the sweep is wired."
    ),
    strict=False,
)
...
def test_rid_matches_accepts_frame_without_rid():
    ...
    # Connection-scoped frames (ping, bye) carry no rid; the consumer
