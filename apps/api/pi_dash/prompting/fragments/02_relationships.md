...
- If it cannot, write `Waiting on: <IDs>` in your workpad, post at most one short comment saying what you are waiting for, and end the run as a noop (`pidash run yield --outcome waiting_on_external`). Do not spend the run investigating unrelated things.
{% endif %}
{% endif %}
