...
1. **If the work completed successfully:**
   ...
   - Moved the issue to the correct next state.{% if run.kind != "review" %} **The runner never proactively moves an issue to `completed`/"Done" — read this carefully, it is the single most common place runs get the ending wrong:**
     ...
     - So for the classic examples — "what color is the home page button?" or "help debug why X" — answer fully in an issue comment, then move the issue to **In Review**. Do **not** move it to Done just because you finished answering.{% else %} Resolve the destination from this review pass's outcome — see "Review cycle" Step 3: an **approved** review posts its summary and **leaves the issue In Review** (the runner never moves it to Done); **changes needed** or a **clarification** question follow "Blocking the run"; a **noop** exits without moving state. Match the target `group` first in "Available states", then the name.{% endif %}
