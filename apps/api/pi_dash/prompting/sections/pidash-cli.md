...
- `pidash issue list --project <PROJ-or-UUID> [--cursor <c>] [--per-page <N>] [--order-by <field>] [--parent <PROJ-123-or-UUID>]` — list work items in a project. `--project` accepts either the workspace-scoped slug (e.g. `ENG`) or a project UUID. `--parent` (a `PROJ-123` identifier or a parent UUID) lists only that parent's **children** — use it to find the child issues an earlier run created before you split, so you never file duplicates. Returns the server's paginated envelope `{count, next_cursor, prev_cursor, results: [...]}`; pass `--cursor` from a prior page to walk pages. Use this to find related/duplicate issues in the same project before creating new ones.
- `pidash issue create --project <PROJ-or-UUID> --title "<title>" [--description <s>] [--priority <none|low|medium|high|urgent>] [--state "<state-name-or-UUID>"] [--parent <PROJ-123-or-UUID>]` — file a new work item under the named project. Use this for capturing **discovered scope that does not belong on the current issue** (a follow-up bug, a separated task, a missing prerequisite), or — with `--parent` — for filing a **child issue** when you split an oversized issue task-by-task (see the "Split into child issues" outcome in "Analyze & scope"). `--parent` attaches the new item as a sub-issue of the given issue (project-scoped identifier like `PROJ-123`, or a raw UUID); the server validates the parent. Do **not** create an issue to track sub-steps of the current run — use the workpad comment for that. Record the new issue's identifier in the workpad so the operator can find it.
