...
- `pidash issue wait <identifier>` — record that you have read this issue's open blockers, decided you cannot safely proceed, and are yielding. It buys back the tick this run is spending, so waiting costs no net budget: call it, then `pidash run yield --outcome waiting_on_external`. Say *why* in your workpad first — nothing on the platform reads that note, but the next tick's you will. A refusal is a normal exit 0; read `applied` and `reason` in the printed `{applied, reason, used, granted, waited, cap}`. A transport or permission error exits non-zero like any other CLI call — yield anyway rather than retrying in a loop. A refusal (`wait_cap_reached` once the one-pool allowance is spent, `infinite_pool` when the issue has no cap, `no_ticker`) changes nothing and means further waiting spends normal budget — tell the human in a comment rather than looping on it. Calling it twice in one run buys two ticks while the allowance lasts.
