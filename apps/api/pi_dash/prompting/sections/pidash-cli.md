...
- `pidash issue list --project <PROJ-or-UUID> [--cursor <c>] [--per-page <N>] [--order-by <field>] [--parent <identifier>]` — list work items in a project. `--project` accepts either the workspace-scoped slug (e.g. `ENG`) or a project UUID. `--parent <identifier>` (a `PROJ-N` identifier or a parent UUID) lists only that parent's **children** — use it to find the child issues an earlier run created before you split, so you never file duplicates. Returns the server's paginated envelope `{count, next_cursor, prev_cursor, results: [...]}`; pass `--cursor` from a prior page to walk pages. Use this to find related/duplicate issues in the same project before creating new ones.
- `pidash issue create --project <PROJ-or-UUID> --title "<title>" [--description <s>] [--priority <none|low|medium|high|urgent>] [--state "<state-name-or-UUID>"] [--parent <identifier>]` — file a new work item under the named project. Use this for capturing **discovered scope that does not belong on the current issue** (a follow-up bug, a separated task, a missing prerequisite), or — with `--parent <identifier>` (a `PROJ-N` identifier or a parent UUID) — for filing a **child issue** when you split an oversized issue task-by-task (see the "Split into child issues" outcome in "Analyze & scope"). Do **not** create an issue to track sub-steps of the current run — use the workpad comment for that. Record the new issue's identifier in the workpad so the operator can find it.
...
- `pidash issue patch <identifier> [--title <s>] [--description <s>] [--priority <none|low|medium|high|urgent>] [--parent <identifier>]` — update other fields. At least one flag is required. `--parent <identifier>` (a `PROJ-N` identifier or a parent UUID) re-parents this issue under the given parent. Do **not** edit title or description for planning or progress tracking — use the workpad comment instead.
