...
6. **Decision gate. Separate two independent questions — *is the direction clear?* and *does the work fit one PR or many?* — then choose exactly one path:**
   ...
   - **Split into child issues** — the issue is really **several genuinely separate tasks**: independent deliverables that belong in **different issues**. **Split on independence, never on size.** You have no run timeout or turn cap, so a large change is not by itself a reason to split — one long run with one review is usually cheaper and safer than several children. Crossing layers is not a reason either: the backend, frontend and CLI of **one feature** share a contract, and splitting them lets each side build against its own guess of that contract — that is how a feature ships with every part green and the whole thing broken. Parts of one feature stay on this issue and follow the multi-part outcome above.
     ...
     - **Record the children in this parent's description.** Once they exist, rewrite the parent's description to keep its original scope and acceptance criteria *and* add the child list — identifier, title, what each depends on, and the order they run in: read it with `pidash issue get {{ issue.identifier }}`, then `pidash issue patch {{ issue.identifier }} --description "<original text + the child list>"`. **Send plain markdown, never the HTML you just read.** `issue get` returns the description only as `description_html` (markup); if you paste that straight back into `--description` the server escapes it and the description turns into visible `&lt;p&gt;` source — and it is escaped again on every later rewrite. Convert it back to plain markdown first, then append the child list. This is the one place you *do* edit a description: the child list is the parent's scope now, and a child's run reads its parent's description to recognise a tracking parent (a comment is not enough — the child only sees the parent's description).
     ...
     - **Record the order as relations.** For each child that must wait on a sibling, run
       `pidash issue relate <later-child> --blocked-by <earlier-child>` so the dependent sees its
       blockers and their states in its own run instead of racing ahead, and so it is woken as soon
       as a blocker completes. Leave independent children unrelated.
