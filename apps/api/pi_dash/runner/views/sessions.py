...
async def runner_session_poll(request, runner_id, sid):
    ...
    ``database_sync_to_async``); only the wait is async.

    The helper must be ``database_sync_to_async`` rather than a bare
    ...
    try:
        runner = await database_sync_to_async(_authenticate_poll_runner)(request)
    ...
    error, plan = await database_sync_to_async(_poll_bookkeeping)(runner, body, sid)
    ...
    if plan["use_zero"]:
        await database_sync_to_async(outbox.mark_pel_drained)(sid)
