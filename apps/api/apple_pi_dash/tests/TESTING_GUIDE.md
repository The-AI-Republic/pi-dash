# Testing Guide for Apple Pi Dash

This guide explains how to write tests for Apple Pi Dash using our pytest-based testing strategy.
...
```python
import pytest
from apple_pi_dash.api.serializers import MySerializer
...
Smoke tests should be placed in `tests/smoke/` directory and use the `apple_pi_dash_server` fixture to test against a real HTTP server.
...
class TestCriticalFlow:
    @pytest.mark.django_db
    def test_login_flow(self, apple_pi_dash_server, create_user, user_data):
        # Get login URL
        url = f"{apple_pi_dash_server.url}/api/auth/signin/"
...
```python
from apple_pi_dash.tests.factories import UserFactory, WorkspaceFactory
...
python -m pytest -m unit --cov=apple_pi_dash
```
