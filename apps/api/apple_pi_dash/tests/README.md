...
This directory contains tests for the Apple Pi Dash application. The tests are organized using pytest.
...
Apple Pi Dash has two types of API endpoints:

1. **External API** (`apple_pi_dash.api`):
...
2. **Web App API** (`apple_pi_dash.app`):
...
```bash
# Run unit tests
python -m pytest apple_pi_dash/tests/unit/

# Run API contract tests
python -m pytest apple_pi_dash/tests/contract/api/

# Run App contract tests
python -m pytest apple_pi_dash/tests/contract/app/

# Run smoke tests
python -m pytest apple_pi_dash/tests/smoke/
```
...
- `apple_pi_dash_server`: Live Django test server for HTTP-based smoke tests
...
2. Use the correct client fixture based on the API being tested:
   ...
   - For smoke tests with real HTTP, use `apple_pi_dash_server`
...
```bash
python -m pytest --cov=apple_pi_dash --cov-report=term --cov-report=html
```
