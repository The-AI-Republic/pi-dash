...
from apple_pi_dash.db.models import User, Workspace, WorkspaceMember
...
def session_client(api_client, create_user):
    """Return a session authenticated API client for app API testing, which is what apple-pi-dash.app uses"""
...
def create_bot_user(db):
    ...
    user = User.objects.create(
        email=f"bot-{unique_id}@apple-pi-dash.so",
...
@pytest.fixture
def apple_pi_dash_server(live_server):
    """
