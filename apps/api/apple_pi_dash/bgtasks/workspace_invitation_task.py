...
# Module imports
from apple_pi_dash.db.models import User, Workspace, WorkspaceMemberInvite
from apple_pi_dash.license.utils.instance_value import get_email_configuration
from apple_pi_dash.utils.email import generate_plain_text_from_html
...
def workspace_invitation(email, workspace_id, token, current_site, inviter):
    try:
        ...
        subject = f"{user.first_name or user.display_name or user.email} has invited you to join them in {workspace.name} on Apple Pi Dash"  # noqa: E501
        ...
        msg.send()
        logging.getLogger("apple_pi_dash.worker").info("Email sent successfully")
        return
